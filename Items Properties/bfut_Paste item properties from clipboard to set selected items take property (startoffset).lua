--[[
  @author bfut
  @version 1.1
  @description bfut_Paste item properties from clipboard to set selected items take property (startoffset)
  @about
...
for i = 0, COUNT_SEL_ITEMS - 1 do
  ...
  if not IS_ITEM_LOCKED[reaper.GetMediaItemInfo_Value(item, "C_LOCK")] then
    if CONFIG["mode"] == "item" then
      reaper.SetMediaItemInfo_Value(item, CONFIG["property"], newvalue)
    else
      local take = reaper.GetActiveTake(item)
      if take then
        reaper.SetMediaItemTakeInfo_Value(take, CONFIG["property"], newvalue)
      end
    end
  end
end
...
reaper.Undo_EndBlock2(0, "bfut_Paste item properties from clipboard to set selected items take property (startoffset).lua", -1)