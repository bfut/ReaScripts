--[[
  @author bfut
  @version 1.2
  @description bfut_MIDI notes to empty items (notes to subtrack, note pitch as item rate)
  @about
...
function bfut_FetchMIDI_notes(take, default_velocity)
  ...
  for i = 0, select(2, reaper.MIDI_CountEvts(take)) - 1 do
    note = {reaper.MIDI_GetNote(take, i)}
    ...
    while note[4] < item_end do
      if note[4] < item_end and note[5] > item_start then
        note[4] = math.max(note[4], item_start)
        note[5] = math.min(note[5], item_end)
        notes[#notes + 1] = note
      end
      note = {note[1], note[2], note[3], note[4], note[5], note[6], note[7], note[8]}
      note[4] = note[4] + take_length
      note[5] = math.min(note[5] + take_length, item_end)
    end
  end
  return notes
end
